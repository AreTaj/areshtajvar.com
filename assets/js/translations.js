const translations = {
    en: {
        ...
        exp_tajvar_b2: "Architected the predictive system using on-device machine learning to deliver personalized forecasts with zero remote data transmission.",
        exp_tajvar_b3: "Integrated passive health and environmental sensor data to inform the forecasting model without requiring manual user input for those signals.",
        exp_tajvar_b4: "Designed a custom user interface specifically for users experiencing acute neurological symptoms, prioritizing accessible interaction patterns.",
        exp_tajvar_b5: "Conducted market research, competitive analysis, and commercialization planning to evaluate viable business pathways.",
        ...
        rag_demo_status: '<span style="color: var(--text-primary); font-weight: 600;">Note:</span> The interactive demo opens in a new tab on Hugging Face Spaces.',
        ...
        migraine_p6: "To maintain clear software boundaries, all analytical risk projections and pattern insights operate strictly within non-diagnostic informational parameters, structuring data around personal self-monitoring and environmental correlation.",
        migraine_r1: "The application operates on a zero-telemetry local architecture, ensuring all health analytics and biometric storage remain strictly on-device.",
        migraine_r2: "Native ingestion pipelines integrate Apple HealthKit and WeatherKit to correlate biometric strain with localized atmospheric pressure shifts.",
        migraine_r3: "The photophobia-calibrated SwiftUI interface eliminates motion triggers and visual glare for pain-free logging during acute episodes.",
        migraine_r4: "Longitudinal analytics built with Swift Charts render 24-hour risk curves, 7-day environmental outlooks, and episode trends with full CSV portability.",
        migraine_t1: "Local SwiftData persistence keeps symptom logging fully functional offline and removes server hosting maintenance.",
        migraine_t2: "Static interface controls prevent triggering visual aura or nausea during acute photophobia, where typical iOS fluid animations cause discomfort.",
        migraine_t3: "Passive WeatherKit and HealthKit background queries automate atmospheric pressure and heart rate logging, eliminating the need for manual context entry.",
        ...
        rag_r1: "Domain adaptation and multi-architecture benchmarking increased retrieval precision by 140%, reaching a 72% Hit Rate @ 5 across municipal code queries.",
        rag_r2: "A multi-metric evaluation pipeline combining BERTScore and LLM-as-a-judge scoring quantifiably penalizes and mitigates hallucinations.",
        rag_r3: "I architected and deployed the complete application to Hugging Face Spaces with an open-source domain-adapted model and interactive public interface.",
        ...
        eurosat_r1: "The pipeline classifies satellite imagery across 10 distinct terrain categories using a multi-class convolutional architecture.",
        eurosat_r2: "Data augmentation techniques expand the effective training distribution, improving model generalization across novel geographic regions.",
        eurosat_r3: "Confusion matrix analysis verifies clear class boundary separation across visually similar terrain categories.",
        ...
        opt_r1: "The application implements the Black-Scholes stochastic differential equation natively within a vectorized Python computation engine.",
        opt_r2: "An interactive Tkinter desktop interface allows users to adjust market parameters and inspect theoretical option premiums in real time.",
        opt_r3: "Closed-form calculations determine theoretical Call and Put values simultaneously with immediate parameter sensitivity feedback.",
        // Strata Advisor
        ...
        strata_r1: "The MetaAllocator scales total portfolio exposure dynamically, deleveraging into cash during elevated volatility regimes.",
        strata_r2: "The strategic engine scores multi-asset universes through Top-N relative momentum and inverse-volatility weighting.",
        strata_r3: "Tactical machine learning ensembles receive dynamic risk budgets funded solely from unallocated strategic cash.",
        strata_r4: "Rebalance logic evaluates lot-level embedded gains and conviction hurdles to prevent turnover that generates net tax drag.",
        strata_r5: "A local terminal staging dashboard presents all generated trade intents for explicit human review and approval prior to submission.",
        strata_t1: "Lot-level tax accounting protects profitable core holdings from being churned during marginal rebalances that would otherwise generate taxable capital gains.",
        strata_t2: "The system redeploys unallocated strategic cash into tactical ML mean-reversion models whenever the volatility governor dials down equity exposure.",
        strata_t3: "The non-discretionary terminal interface prevents unauthorized order execution by requiring explicit manual approval before any staged trade is submitted.",
        ...
        adsb_r1: "The ingestion pipeline continuously queries the OpenSky Network API and maintains stateful trajectory records in a local SQLite database.",
        adsb_r2: "An unsupervised Isolation Forest model flags GPS jamming and spoofing anomalies across multi-dimensional kinematic features.",
        adsb_r3: "Interactive Folium maps render spatial flight paths with color-coded classifications distinguishing normal flight corridors from spoofed trajectories.",
        // Swarm Simulator
    ...
    es: {
        ...
        exp_tajvar_b2: "Diseñé la arquitectura del sistema predictivo mediante aprendizaje automático en el dispositivo para generar pronósticos personalizados con cero transmisión remota de datos.",
        exp_tajvar_b3: "Integré datos pasivos de sensores de salud y variables ambientales para alimentar el modelo predictivo sin requerir la entrada manual del usuario.",
        exp_tajvar_b4: "Diseñé una interfaz de usuario personalizada específicamente para personas que experimentan síntomas neurológicos agudos, priorizando patrones de interacción accesibles.",
        exp_tajvar_b5: "Realicé estudios de mercado, análisis competitivo y planes de comercialización para evaluar modelos de negocio viables.",
        exp_ng: "Northrop Grumman Corporation",
        ...
        rag_demo_status: '<span style="color: var(--text-primary); font-weight: 600;">Aviso:</span> La demo interactiva se abre en una pestaña nueva en Hugging Face Spaces.',
        ...
        migraine_p3: "Tras una fase inicial de desarrollo de prototipos en Python para modelar y validar los algoritmos de pronóstico, desarrollé Migraine Navigator como una aplicación de producción nativa para iOS y iPadOS utilizando <strong>Swift</strong>, <strong>SwiftUI</strong> y <strong>SwiftData</strong>. El sistema está diseñado bajo un estricto modelo de privacidad en el dispositivo: todos los registros fisiológicos, coordenadas de ubicación y cómputos analíticos se ejecutan localmente sin telemetría remota ni procesamiento en la nube.",
        ...
        migraine_p5: "La interfaz está diseñada específicamente para la accesibilidad ante la fotofobia, sustituyendo animaciones fluidas por controles estáticos de alto contraste.",
        migraine_p6: "Para mantener límites claros de software, todas las proyecciones analíticas de riesgo e identificación de patrones operan estrictamente dentro de parámetros informativos no diagnósticos, organizando los datos en torno al autocontrol personal y la correlación ambiental.",
        migraine_r1: "La aplicación opera sobre una arquitectura local sin telemetría, garantizando que el almacenamiento y análisis biométrico permanezcan estrictamente en el dispositivo.",
        migraine_r2: "Flujos de integración nativos conectan Apple HealthKit y WeatherKit para correlacionar el esfuerzo biométrico con variaciones de presión atmosférica.",
        migraine_r3: "La interfaz en SwiftUI está calibrada para fotofobia, eliminando transiciones con movimiento y destellos visuales para facilitar el registro durante crisis de dolor.",
        migraine_r4: "El módulo analítico longitudinal desarrollado con Swift Charts proyecta curvas de riesgo de 24 horas, previsiones ambientales de 7 días y tendencias históricas con exportación CSV.",
        migraine_t1: "La persistencia local en SwiftData mantiene el registro de síntomas completamente operativo sin conexión y elimina los costos de mantenimiento de servidores.",
        migraine_t2: "Los controles estáticos de la interfaz previenen la activación de aura visual o náuseas durante crisis de fotofobia, donde las animaciones fluidas habituales de iOS generan malestar.",
        migraine_t3: "Las consultas en segundo plano de WeatherKit y HealthKit automatizan la captura de presión atmosférica y frecuencia cardíaca, eliminando la necesidad de entrada manual de contexto.",
        ...
        rag_r1: "La adaptación al dominio y la comparativa multiarquitectura incrementaron la precisión de recuperación en un 140%, alcanzando un 72% de Hit Rate @ 5 en consultas normativas.",
        rag_r2: "Un flujo de evaluación métrica con BERTScore y jueces LLM penaliza y mitiga cuantitativamente las tasas de alucinación.",
        rag_r3: "Diseñé y desplegué la aplicación completa en Hugging Face Spaces con un modelo adaptado al dominio de código abierto y una interfaz pública interactiva.",
        ...
        eurosat_r1: "El pipeline clasifica imágenes satelitales en 10 categorías de terreno utilizando una arquitectura convolucional multiclase.",
        eurosat_r2: "Técnicas de aumento de datos amplían la distribución de entrenamiento, mejorando la generalización del modelo ante regiones geográficas no vistas.",
        eurosat_r3: "El análisis de matriz de confusión verifica la separación precisa de fronteras entre categorías de terreno visualmente similares.",
        ...
        opt_r1: "La aplicación ejecuta la ecuación diferencial estocástica de Black-Scholes de forma nativa mediante un motor de cómputo vectorizado en Python.",
        opt_r2: "Una interfaz de escritorio interactiva en Tkinter permite a los usuarios ajustar parámetros de mercado y consultar primas teóricas en tiempo real.",
        opt_r3: "Cálculos analíticos de forma cerrada determinan simultáneamente los valores de opciones Call y Put con respuesta inmediata a la variación de parámetros.",
        // Strata Advisor
        ...
        strata_r1: "El MetaAllocator escala dinámicamente la exposición total de la cartera, reduciendo el apalancamiento hacia efectivo durante regímenes de alta volatilidad.",
        strata_r2: "El motor estratégico evalúa universos multi-activo mediante momentum relativo Top-N y ponderación por inversa de volatilidad.",
        strata_r3: "Modelos tácticos de aprendizaje automático reciben presupuestos de riesgo dinámicos financiados únicamente con capital estratégico no asignado.",
        strata_r4: "La lógica de rebalanceo evalúa ganancias acumuladas por lote y umbrales de convicción para evitar rotaciones que generen fricción fiscal.",
        strata_r5: "Un panel de control local en terminal presenta todas las intenciones de orden generadas para revisión y aprobación humana explícita antes del envío.",
        strata_t1: "La contabilidad fiscal por lote protege las posiciones ganadoras principales de ser rotadas en rebalanceos marginales que generarían ganancias de capital imponibles.",
        strata_t2: "El sistema reasigna el efectivo estratégico no utilizado a modelos tácticos de reversión a la media cuando el gobernador de volatilidad reduce la exposición a renta variable.",
        strata_t3: "La interfaz de terminal no discrecional previene la ejecución no autorizada al exigir aprobación manual explícita antes de enviar cualquier orden preparada.",
        // ADS-B Anomaly Detector
        project_adsb_tag: "SIGINT · ML",
        ...
        adsb_r1: "El pipeline de ingesta consulta continuamente la API de OpenSky Network y mantiene registros de trayectoria con estado en una base de datos SQLite local.",
        adsb_r2: "Un modelo de Isolation Forest no supervisado detecta anomalías de interferencia y suplantación de GPS sobre variables cinemáticas multidimensionales.",
        adsb_r3: "Mapas interactivos en Folium representan trayectorias espaciales con clasificaciones por colores para distinguir corredores normales de rutas anómalas.",
        // Swarm Simulator
